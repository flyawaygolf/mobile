...
import { searchAll, searchUsers, searchGolfs, searchEvents } from './Interfaces/Search';
...
class SearchManager extends RequestEmitter {
  ...
  public async events(query: string, options?: {
    location?: LocationQuery
  }) {
    let _url = `/search/events`;
    const parameters = []
    ...
    const request = await this.getRequest(_url.concat(parameters.join("&")));
    const response = request as searchEvents;
