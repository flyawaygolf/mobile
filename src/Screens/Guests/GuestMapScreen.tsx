...
const GuestMapScreen = () => {
  ...
  const [modalInfo, setModalInfo] = useState<{ visible: boolean; data?: userInfo | golfInterface }>({ visible: false });
  ...
  }, [mapRef, filter, location, setLoadingCenter, t, handleChipPress]);
  ...
  const userMarkers = useMemo(() => users.map((u, idx) => {
    return (
      <Marker
        key={idx}
        onPress={() => {
          setModalInfo({ visible: true, data: u })
        }}
        coordinate={{
  ...
  const golfMarkers = useMemo(() => golfs.map((g, idx) => {
    ...
    return (
      <Marker
        ...
        onPress={() => setModalInfo({ visible: true, data: g })}
