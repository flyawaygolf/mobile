...
type PropsType = {
    ...
    visible: boolean;
    hideModal: () => void;
    setModif: (newHcp: number) => void;
    min?: number;
    max?: number;
}

const HandicapModal = ({ handicap, visible, hideModal, setModif, min, max }: PropsType) => {
            ...
            <Dialog visible={visible} onDismiss={hideModal}>
                ...
                <Dialog.ScrollArea style={{ paddingHorizontal: 0, flexDirection: "row" }}>
                    <ScrollView style={{ paddingHorizontal: 24, height: 200, margin: 10 }}>
                        {
                            handicapNumbersInt
                                .filter(v => (!min || v <= min / 10) && (!max || v >= max / 10))
                                .map((v, idx) => (
                    ...
                    <ScrollView style={{ paddingHorizontal: 24, height: 200, margin: 10 }}>
                        {
                            handicapNumbersFloat
                                .filter(v => {
                                    const totalValue = hcp.int + v;
                                    return (!max || totalValue >= max / 10) && (!min || totalValue <= min / 10);
                                })
                                .map((v, idx) => (
