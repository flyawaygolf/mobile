...
const DisplayNotifications = ({ info, readOneNotification }: sectionProps) => {
    ...
    const navigateScreen = async (notification_type: notificationTypeInterface) => {
        ...
        switch (notification_type) {
            ...
            case "shares":
                return navigation.navigate("PostsStack", { screen: "PostScreen", params: { post_id: info?.post?.post_id } })
            case "events":
                return navigation.navigate("EventStack", { screen: "DisplayEventScreen", params: { event_id: info?.event?.event_id }})
            case "events_modifications":
                return navigation.navigate("EventStack", { screen: "DisplayEventScreen", params: { event_id: info?.event?.event_id } })
            default:
                return ""
        }
    ...
    const displayedText = () => {
        ...
        switch (info.notification_type) {
            ...
            case "events":
                return t("notification.event", { username: user.username, title: event_info?.title, golf: golf_info?.name ?? "..." });
            case "events_modifications":
                return t("notification.event_modification", { username: user.username, title: event_info?.title, golf: golf_info?.name ?? "..." });
            default:
                return ""
        }
    }
